...
async function withRpcRetry<T>(operation: () => Promise<T>): Promise<T> {
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      const rateLimited = message.includes("Too Many Requests") || message.includes("-32005");
...
export function PharmaWalletView({ mode }: { mode: ViewMode }) {
  ...
  const [, setStatus] = useState("Connect your wallet to review the chain");
  ...
  useEffect(() => {
    ...
    // The refresh function intentionally uses the stable read-only contract instance.
  ...
  async function fetchAllUnits(contract: ethers.Contract): Promise<UnitRecord[]> {
    ...
    const activity = await fetchUnitActivity(contract);
    for (const unit of unitsList) Object.assign(unit, activity[unit.id]);
    return unitsList;
  }
      ...
      if (action === "initiate") {
        ...
        try {
          [receiverIsManufacturer, receiverIsHandler] = await Promise.all([
            contract.isManufacturer(actionReceiver.trim()),
            contract.isHandler(actionReceiver.trim()),
          ]);
        } catch (error) {
